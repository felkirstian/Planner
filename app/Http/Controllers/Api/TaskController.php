...
class TaskController extends Controller
{
    ...
    {
        try {
            \Log::info("Starting task deletion for task ID: {$task->id}");
            ...
            $user = Auth::user();
            \Log::info("User {$user->id} attempting to delete task {$task->id}");

            if ($user->id !== $task->user_id) {
                \Log::error("Unauthorized deletion - User {$user->id} tried to delete task {$task->id} owned by {$task->user_id}");
                return response()->json([
                    'status' => 'error',
            ...
            $deleted = $task->delete();
            \Log::info("Task deletion result: " . ($deleted ? "Success" : "Failed") . " for task {$task->id}");
            ...
            if ($deleted) {
                ...
                \Log::error("Failed to delete task {$task->id}");
                return response()->json([
                    'status' => 'error',
            ...
            \Log::error("Exception during task deletion: " . $e->getMessage());
            return response()->json([
                'status' => 'error',
